  ...
  return (
    <>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        ...
        <section className="rounded-[22px] bg-primary p-4 text-white"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-white/45">Daily delivery</p><h2 className="mt-2 text-lg font-extrabold">Your numbers, in your inbox.</h2><p className="mt-2 max-w-md text-[11px] leading-5 text-white/55">Send a clean stock brief to {workspace.settings.reportEmail}. The report is generated from the current database at send time.</p><button type="button" disabled={sending} onClick={() => void send()} className="mt-5 flex min-h-11 items-center gap-2 rounded-[14px] bg-accent px-4 text-[11px] font-extrabold text-accent-ink"><Mail className="size-4" /> {sending ? "Sending report..." : "Email stock brief"}</button></section>
      </div>
    </>
  );
}
