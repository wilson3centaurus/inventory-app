    ...
    <main className="grid min-h-dvh bg-primary lg:grid-cols-[1.05fr_.95fr]">
      ...
      <section className="flex min-h-dvh items-center justify-center rounded-t-[32px] bg-background px-4 py-8 lg:rounded-l-[36px] lg:rounded-tr-none">
        <div className="w-full max-w-[390px]">
          <div className="mb-8 flex items-center gap-3 lg:hidden"><span className="grid size-10 place-items-center rounded-[15px] bg-accent text-primary"><Sparkles className="size-5" /></span><div><p className="text-base font-extrabold">StockFlow</p><p className="text-[9px] uppercase tracking-[.15em] text-muted">The shop operating system</p></div></div>
          ...
          <form onSubmit={submit} className="mt-6 space-y-3">
            ...
            <label className="block space-y-1.5"><span className="text-[10px] font-bold text-muted">Password</span><div className="relative"><LockKeyhole className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><input required minLength={8} type="password" autoComplete={mode === "setup" ? "new-password" : "current-password"} className="field w-full rounded-[14px] pl-9 pr-3 text-xs" placeholder="At least 8 characters" value={form.password} onChange={(event) => setForm((current) => ({ ...current, password: event.target.value }))} /></div></label>
